...
export default function HomeMisiKami() {
  return (
    <section className="mt-30 w-full min-h-[520px] bg-cover bg-no-repeat bg-center relative overflow-hidden flex items-center justify-center" style={{ backgroundImage: `url(${Background})` }}>
      <div className="w-full flex flex-col justify-center items-center sm:py-8 py-4 px-2 sm:px-6">
        <h2 className="font-extrabold text-xl sm:text-2xl md:text-3xl lg:text-[48px] text-white text-center mb-4 sm:mb-6">Misi Kami</h2>
          ...
          {missionItems.map((item, index) => (
            <div
              className={
                ...
                (missionItems.length % 2 === 1 && index === missionItems.length - 1 ? " sm:col-span-2 sm:justify-self-center lg:col-span-1 lg:justify-self-auto" : "")
              }
              key={item.number}
            >
              <h3
              ...
              <p className={"text-center mx-auto mt-2 break-words whitespace-normal text-sm sm:text-base md:text-lg lg:text-xl max-w-[90vw] sm:max-w-[220px] md:max-w-[305px] text-white"}>{item.text}</p>
            </div>
          ))}
        </div>
      </div>
